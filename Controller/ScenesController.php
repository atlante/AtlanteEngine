...
class ScenesController extends AppController {
  ...
  public function game_index() {
    if ($this->Session->check('Player')) {
      $this->set('scene', current($this->Scene->find('first', [
                          'conditions' => array('Scene.id' => $this->Session->read('Player.scene_id'))
      ])));
    } else {
      $this->redirect(array('controller' => 'players', 'action' => 'select'));
    }
  }
