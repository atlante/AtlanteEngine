...
//to
    ...
    public function game_view($id = null) {
        App::uses('File', 'Utility');
        $story = new File(WWW_ROOT . 'files' . DS . 'playersStory' . DS . $id . '.md');
        if (is_null($id)) {
            $id = $this->Session->read('Player.id');
            $this->set('private', TRUE);
        } else {
            $this->set('private', FALSE);
        }
        ...
        if ($story->exists()) {
            $player['Player']['story'] = $story->read();
        }else{
            $player['Player']['story'] = 'Contacte Agentcobra avec ta desciption dans un fichier .txt';
        }
